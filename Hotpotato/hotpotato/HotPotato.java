...
public class HotPotato {
    ...
    else{for(int i=0;i<MAX_PLAYERS;i++){this.circle.append(players.get(i));}}
     ...
     */
    public void playGame(){String state="RIGHT";
        while(circle.size()>1){int c=getRandomNumPasses();
            display();
        System.out.println("Passing potato "+c+" times "+state+":");
        while(c-->0){
            System.out.println("\t:"+circle.get().toString()+" has potato and is passing "+state);
        if(state.equals("RIGHT")){circle.forward();}
        else{circle.backward();}
        }
        System.out.println(circle.get().toString()+ " lost!\n");
        if(state.equals("RIGHT")&&circle.size()>1){
        circle.removeForward();
        }
        else if(circle.size()>1){
        circle.removeBackward();
        }
        state=(state.equals("RIGHT"))?"LEFT":"RIGHT";
        this.round++;
        }
        display();
        System.out.println(circle.get().toString()+" wins!");
    }
